...
if __name__ == "__main__": 
    ...
    # TODO: Insert other useful sanity checks (application specific).
    # ------------------------------------------------------------------------: 
    # ------------------------------------------------------------------------: 
    # Create output CSV column headers:
    ...
    # Prep first row instance
    row_instances = []
    row_instance = OrderedDict()
    for ch in column_headers: row_instance[ch] = 0
                ...
                for cmdno, sqpg in port.seq_prog.program.items(): 
                    if sqpg['instr'] == "CALL": 
                        ...
                        if call_main_label in main_labels: pass
                            ...
                            if len(__labels) > 1: 
                                err_msg = "More than one label return: %s"%(call_main_label)
                                error_log(setup_reference,err_msg,__labels)
